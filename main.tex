...
\textbf{Research hypotheses} (\textit{ceteris paribus}):

\bigskip

\textbf{H1 — Returns to Experience.}
Professional experience is positively correlated with earnings, reflecting accumulated
on-the-job human capital. This is expected to be statistically significant ($\beta_2 > 0$,
$p < 0.05$) across all three models.

\textbf{H2 — Education Premium.}
Higher education levels (Bachelor, Master, PhD) yield significantly higher earnings
compared to the High School baseline. Returns are hypothesised to be monotonically
increasing with attainment level, consistent with human capital theory \citep{becker1964}.

\textbf{H3 — Non-Linear Age Effects.}
Age exhibits an inverted U-shape relationship with earnings, with positive returns in
early career stages and diminishing marginal returns at later stages. This hypothesis is
testable \textit{exclusively} in Model~3 via the coefficient on $\text{Age}_c^2$
(expected: $\beta_2 < 0$, $p < 0.05$).

\textbf{H4 — Conditional Returns to Experience by Education.}
The returns to experience are conditional on education level, with higher-attainment
groups potentially exhibiting steeper wage growth trajectories. This is testable in
Model~3 via the interaction terms $(\text{Edu}_j \times \text{Exp})$; a positive and
significant $\hat{\delta}_j$ for higher education groups would support H4.
...
\subsection{Appendix A: Full Regression Output}
\begin{table}[htpb]
\centering
\scriptsize
\caption{Model 2 HC3 Regression Output}
\begin{tabular}{lrrrrr}
\toprule
\textbf{Variable} & \textbf{Coef.} & \textbf{SE} & \textbf{t} & \textbf{p-value} & \textbf{95\% CI} \\
\midrule
Intercept & 10.7187 & 0.153 & 69.948 & 0.000 & [10.417, 11.020] \\
Male & 0.0629 & 0.030 & 2.073 & 0.039 & [0.003, 0.123] \\
Other & 0.1073 & 58.857 & 0.002 & 0.999 & [-115.587, 115.801] \\
Bachelor & 0.6943 & 0.051 & 13.719 & 0.000 & [0.595, 0.794] \\
Master & 0.8468 & 0.056 & 15.231 & 0.000 & [0.737, 0.956] \\
PhD & 0.8554 & 0.064 & 13.335 & 0.000 & [0.729, 0.981] \\
...
\noindent\textit{Model summary:} $R^2 = 0.713$, adjusted $R^2 = 0.709$, $F = 158.1$, $p < 0.001$, AIC $= 189.2$, BIC $= 221.6$, $n = 425$, HC3 robust SE, Jarque--Bera $= 0.184$ ($p = 0.912$), Durbin--Watson $= 1.748$.

\subsection{Appendix B: Outlier Report (Filtered)}
\begin{table}[htpb]
\centering
\scriptsize
...
\subsection{Appendix C: Python Source Code (Data Cleaning Pipeline)}
...
\subsection{Appendix D: Python Source Code (OLS Estimation with HC3)}
\begin{lstlisting}[language=Python,basicstyle=\ttfamily\scriptsize,breaklines=true]
import pandas as pd
import numpy as np
