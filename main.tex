...
\begin{equation}
  \text{Salary}_i = \beta_0 + \beta_1\,\text{Age}_i + \beta_2\,\text{Exp}_i + \beta_3\,\text{Gender}_i + \beta_4\,\text{Education}_i + u_i
  \label{eq:m1}
\end{equation}

Where $i$ denotes the individual observation, and $\text{Gender}_i$ and $\text{Education}_i$ represent the sets of dummy variables for categorical characteristics. This model establishes a benchmark and is interpretable in levels (USD). Its primary
...
\begin{equation}
  \ln(\text{Salary}_i) = \beta_0 + \beta_1\,\text{Age}_i + \beta_2\,\text{Exp}_i + \beta_3\,\text{Gender}_i + \beta_4\,\text{Education}_i + u_i
  \label{eq:m2}
\end{equation}
...
\begin{equation}
\begin{aligned}
  \ln(\text{Salary}_i) ={}& \beta_0 + \beta_1\,\text{Age}_{c,i} + \beta_2\,\text{Age}_{c,i}^2 + \beta_3\,\text{Exp}_i + \beta_4\,\text{Gender}_i \\
    &+ \beta_5\,\text{Education}_i + \beta_6\,(\text{Education}_i \times \text{Exp}_i) + u_i
\end{aligned}
  \label{eq:m3}
\end{equation}
...
interaction terms $(\text{Education}_i \times \text{Exp}_i)$ allow the marginal return to
...
\end{itemize}
$H_0: \beta_{\text{Age}_c} = \beta_{\text{Age}_c^2} = 0$ (Age has no impact on salary)\\
$H_1: \beta_{\text{Age}_c} > 0$ and $\beta_{\text{Age}_c^2} < 0$ (Salary increases with age but at a diminishing rate)
...
The squared age term (\(\text{Age}_c^2\)) is negative and statistically significant, indicating a concave relationship between age and log salary within this specification. Although the linear term $\text{Age}_c$ is not individually significant, a joint F-test (using the HC3 robust covariance matrix) for $H_0\!: \beta_{\text{Age}_c} = \beta_{\text{Age}_c^2} = 0$ yields $F(2, 413) = 3.90$ ($p = 0.021$), confirming that the age polynomial jointly contributes significant explanatory power at the 5\% level. The age profile should therefore be interpreted based on the joint test rather than the individual coefficient alone.
...
Bachelor & 3.97 & 3.97 & 5.12 \\
Master & 4.01 & 4.01 & 6.84 \\
PhD & 3.84 & 3.84 & 9.15 \\
Age2\_c & --- & --- & 2.38 \\
Exp \(\times\) PhD & --- & --- & 27.95 \\
...
\subsection{Model 1 (Linear) Summary Output}
...
\bottomrule
\end{tabular}\\
\textit{Note: This OLS ANOVA table reports the non-robust F-statistic (136.87). The Regression Statistics table above reports the HC3-robust F-statistic (263.21) used for inference.}
\end{table}
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Model 2 (Semi-Log) Summary Output}
...
\bottomrule
\end{tabular}\\
\textit{Note: This OLS ANOVA table reports the non-robust F-statistic (148.24). The Regression Statistics table above reports the HC3-robust F-statistic (158.11) used for inference.}
\end{table}
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Model 3 (Quadratic -- Final) Summary Output}
...
\subsection{Outlier Report (Filtered)}
\begin{table}[htpb]
\centering
\scriptsize
...
\subsection{Python Source Code (Data Cleaning Pipeline)}
...
def normalize_education(value):
    # Standardise apostrophes (e.g. curly quotes to straight)
...
\subsection{Python Source Code (OLS Estimation with HC3)}
\begin{lstlisting}[language=Python,basicstyle=\ttfamily\scriptsize,breaklines=true]
import pandas as pd
import numpy as np
...
# --- F-test: Model 2 vs Model 3 (nested, Robust) ---
RSS2, RSS3 = sum(m2.resid**2), sum(m3.resid**2)
q = m3.df_model - m2.df_model
F_nested = ((RSS2 - RSS3)/q) / (RSS3/(len(df) - m3.df_model - 1))
p_nested = 1 - stats.f.cdf(F_nested, q, len(df) - m3.df_model - 1)
print(f'F-test M2 vs M3: F({int(q)},{int(len(df)-m3.df_model-1)}) = {F_nested:.4f}, p = {p_nested:.6f}')

# --- Joint F-test for Age terms in Model 3 (HC3 Robust) ---
m3_hc3 = m3.get_robustcov_results(cov_type='HC3')
f_age = m3_hc3.f_test("Agec = 0, Agec2 = 0")
print(f'Joint test Age: F({f_age.df_num},{f_age.df_denom}) = {f_age.fvalue:.4f}, p = {f_age.pvalue:.4f}')
\end{lstlisting}
